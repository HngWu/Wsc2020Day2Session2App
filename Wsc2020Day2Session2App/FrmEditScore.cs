...
namespace Wsc2020Day2Session2App
{
    public partial class FrmEditScore : Form
    {
        ...
        public FrmEditScore()
        {
            ...
            {
                var scoreForCriterion = context.Scores.Where(x => x.CompetitionSubmissionId == submissionId && x.areaCriteriaId == criterion.id).Select(x => x.score1).FirstOrDefault();
                datatable.Rows.Add(criterion.criteria, criterion.maximumMarks, scoreForCriterion);
            }
            ...
            try
            {
                foreach (DataGridViewRow row in dgvsubmissions.Rows)
                {
                    if (i < noOfCriteria)
                    {
                        ...
                        var currentScore = context.Scores.Where(x => x.CompetitionSubmissionId == submissionId && x.areaCriteriaId == areaCriteriaId).FirstOrDefault();

                        currentScore.score1 = Decimal.Parse(score);
                        context.Scores.AddOrUpdate(currentScore);
                        context.SaveChanges();
                        i++;
                    }
